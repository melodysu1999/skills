...
6. Ask whether a one-page Word report should be created always, only when requested, or never by default. Explain that an explicit request can still override the default. Completion: the report policy is recorded without asking the QA user to design document internals.
7. Draft `docs/testing/playbook.md` from confirmed project material using [playbook-template.md](playbook-template.md). Review it section by section. Completion: environment identity, authentication, safe data lifecycle, special workflows, evidence, safety, and verdict rules are each confirmed or explicitly deferred.
8. Build `qa.yaml` according to [contracts.md](contracts.md), then show a readable summary. Completion: the user explicitly approves the summary.
9. Write both files atomically, reread them, resolve every configured relative path against the workspace root, and scan for literal secrets. Completion: required values are present, paths stay inside the workspace, private notes are enforced, and neither file contains a secret.
